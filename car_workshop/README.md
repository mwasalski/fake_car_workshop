...
1. Clone this repo into **Databricks Repos**. The notebooks import `reference_data.py`
   and `table_schemas.py` from the same folder, so the whole `car_workshop/` folder must land together.
...
- **Backfill:** run the job repeatedly with different `TARGET_DATE` values.
...
`SCALE_FACTOR` in `daily.ipynb`: `1.0` ≈ 100K rows/day across all fact tables.
